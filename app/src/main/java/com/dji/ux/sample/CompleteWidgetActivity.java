...
public class CompleteWidgetActivity extends Activity {
    ...
    //直播推流相关属性
    private Button startLiveShowBtn;//罗钦雨 的 推流直播按钮对象初始化
    public String liveShowUrl = "rtmp://149160.livepush.myqcloud.com/live/logic?txSecret=ade07f2f6a9791613f037af83e0958bf&txTime=612DCABE";
        ...
        startLiveShowBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(LiveState==0){
                    ...
                    if(result==0){
                        Toast.makeText(
                                CompleteWidgetActivity.this,
                                "直播推流开始:\n"+ result+
                                        "\n 直播声音是否开启:" + DJISDKManager.getInstance().getLiveStreamManager().isLiveAudioEnabled()+
                                        "\n 当前推流地址："+DJISDKManager.getInstance().getLiveStreamManager().getLiveUrl()+
                                        "\n 直播开始时间:" + DJISDKManager.getInstance().getLiveStreamManager().getStartTime()
                    ...
                    Toast.makeText(CompleteWidgetActivity.this, "直播推流结束", Toast.LENGTH_SHORT).show();
                    LiveState=0;
                    startLiveShowBtn.setBackground(getResources().getDrawable(R.drawable.live_button));
                }

            }
        });
        //设置长按弹出文本框事件
        startLiveShowBtn.setLongClickable(true);
        startLiveShowBtn.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                final EditText inputServer = new EditText(CompleteWidgetActivity.this);
                AlertDialog.Builder builder = new AlertDialog.Builder(CompleteWidgetActivity.this);
                builder.setTitle("输入推流地址").setIcon(android.R.drawable.ic_dialog_info).setView(inputServer)
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(text == null||"".equals(text)||text.length()<=0){
                            Toast.makeText(CompleteWidgetActivity.this, "地址不能为空！", Toast.LENGTH_SHORT).show();
                        }
                        else{
                            //先把推流断开
                            DJISDKManager.getInstance().getLiveStreamManager().stopStream();
